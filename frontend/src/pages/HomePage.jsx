...
const CATEGORIES = [
  { icon: Code2, label: 'IT / Software', color: 'from-blue-500/20 to-blue-600/10 border-blue-500/20 text-blue-400' },
  ...
  { icon: Users, label: 'Human Resources', color: 'from-amber-500/20 to-amber-600/10 border-amber-500/20 text-amber-400' },
  { icon: BarChart3, label: 'Sales', color: 'from-cyan-500/20 to-cyan-600/10 border-cyan-500/20 text-cyan-400' },
  { icon: PenTool, label: 'Design', color: 'from-rose-500/20 to-rose-600/10 border-rose-500/20 text-rose-400' },
...
function FeaturedJobCard({ job }) {
  return (
    <div className="glass-card p-5 hover:border-brand-500/20 hover:-translate-y-1 transition-all duration-300 group">
    ...
    <div className="animate-fade-in">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute top-0 left-1/4 w-[500px] h-[500px] bg-brand-600/15 rounded-full blur-[120px]" />
          <div className="absolute bottom-0 right-1/4 w-[400px] h-[400px] bg-purple-600/15 rounded-full blur-[120px]" />
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[300px] h-[300px] bg-accent-500/8 rounded-full blur-[100px]" />
        </div>

        <div className="section-container relative z-10 py-20 md:py-28">
          <div className="max-w-3xl mx-auto text-center">
            {/* Badge */}
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-brand-500/10 border border-brand-500/20 mb-6 animate-slide-down">
              <Sparkles className="w-3.5 h-3.5 text-brand-400" />
              <span className="text-brand-400 text-xs font-medium">Powered by Sentence-BERT + pgvector</span>
            </div>

            {/* Headline */}
            <h1 className="text-4xl sm:text-5xl md:text-6xl font-bold text-white leading-tight animate-slide-up">
              Tìm việc làm mơ ước
              <br />
              <span className="gradient-text">với sức mạnh AI</span>
            </h1>

            <p className="text-slate-400 text-base sm:text-lg mt-5 max-w-xl mx-auto leading-relaxed animate-slide-up">
            ...
            <div className="mt-4 flex items-center justify-center gap-2 flex-wrap text-xs text-slate-500">
              ...
              {['Data Scientist', 'React Developer', 'DevOps', 'Product Manager'].map(t => (
                <button key={t} onClick={() => { setKeyword(t); navigate(`/jobs?q=${encodeURIComponent(t)}`) }}
                  className="px-2.5 py-1 rounded-full bg-white/5 hover:bg-brand-500/10 hover:text-brand-400 transition-colors cursor-pointer">
                  {t}
                </button>
              ))}
            </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {CATEGORIES.map(({ icon: Icon, label, color }) => (
            <Link
              ...
              key={label}
              className={`glass-card p-5 flex flex-col items-center gap-3 cursor-pointer hover:-translate-y-1 transition-all duration-300 bg-gradient-to-br ${color} border`}
            >
      ...
      <section className="py-16 relative">
        <div className="absolute inset-0 bg-gradient-to-r from-brand-600/5 via-purple-600/5 to-brand-600/5 pointer-events-none" />
            ...
            <div className="flex-1 w-full max-w-md">
              ...
              <div className="space-y-3">
                {[
                  ...
                  <div key={title} className="glass-card p-4 flex items-start gap-3 hover:border-brand-500/20 transition-colors">
      ...
      <section className="section-container py-16">
        <div className="text-center mb-10">
          <h2 className="section-heading">Nền tảng trong những con số</h2>
          <p className="section-subheading">Cộng đồng tìm việc thông minh đang phát triển</p>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { icon: Users, label: 'Người dùng', value: dashboardData?.total_users ?? '100+', color: 'brand' },
            { icon: Briefcase, label: 'Việc làm', value: dashboardData?.total_jobs ?? '500+', color: 'purple' },
            { icon: FileText, label: 'CV đã phân tích', value: dashboardData?.total_cvs ?? '200+', color: 'emerald' },
            { icon: Star, label: 'Gợi ý đã tạo', value: dashboardData?.total_recommendations ?? '1000+', color: 'amber' },
            ...
            return (
              <div key={label} className={`glass-card p-6 bg-gradient-to-br ${colors[color]} border text-center hover:-translate-y-1 transition-all duration-300`}>
                <Icon className="w-7 h-7 mx-auto mb-3 text-white/60" />
      ...
      <section className="section-container py-16">
        <div className="glass-card p-10 md:p-14 text-center relative overflow-hidden">
          <div className="absolute inset-0 bg-gradient-to-r from-brand-600/10 via-purple-600/10 to-brand-600/10 pointer-events-none" />
