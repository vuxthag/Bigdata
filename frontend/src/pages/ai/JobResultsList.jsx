...
function JobCard({ job, rank, cvId }) {
  ...
  return (
    <div className="glow-card p-5 hover:border-brand-500/20 transition-all duration-200 animate-slide-up">
      <div className="flex items-start justify-between gap-3 mb-3">
        <div className="flex items-center gap-3">
          <span className="w-7 h-7 rounded-lg bg-gradient-to-br from-brand-500/30 to-purple-500/20 border border-brand-500/30 flex items-center justify-center text-brand-400 text-xs font-bold flex-shrink-0">#{rank}</span>
          <div>
      ...
      <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <div className="bg-amber-500/5 border border-amber-500/20 rounded-xl p-3">
          ...
          <div className="flex flex-wrap gap-1">
            {missing.map(s => (
              <span key={s} className="inline-flex items-center px-2 py-0.5 rounded-md text-xs bg-amber-500/10 text-amber-400 border border-amber-500/20">{s}</span>
            ))}
          </div>
        </div>
      </div>
